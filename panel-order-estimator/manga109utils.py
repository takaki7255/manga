...
class BoundingBox(object):
    def __init__(self, xmin=None, ymin=None, xmax=None, ymax=None, panels=None, bbtype=None, id_=""):
        self.dict = {
            "@xmin": xmin,
            "@ymin": ymin,
            "@xmax": xmax,
...
class Page(object):
    ...
    def get_image(self):
        # .DS_Storeが文字列に含まれる場合，何も返さない
        if ".DS_Store" in self.parent.get_image_path(self.page_index):
            return None
    ...
    def get_bbs(self):
        bb_dict = dict(
            [
                (a, [BoundingBox(bbtype=a).init_dict(d) for d in self.parent.annotations["page"][self.page_index][a]])
...
class Book(object):
    def __init__(self, title, loader=None, manga109_root_dir="./../Manga109_released_2021_12_30"):
    ...
    def get_image_path(self, i_page):
        bookimgpath = pathlib.Path(self.manga109_root_dir) / "images" / self.title
...
class Manga109Dataset(object):
    def __init__(self, manga109_root_dir="./../Manga109_released_2021_12_30"):
        self.manga109_root_dir = manga109_root_dir
        self.loader = manga109api.Parser(root_dir=manga109_root_dir)
    ...
    def get_book(self, title):
        return Book(title, loader=self.loader, manga109_root_dir=self.manga109_root_dir)
